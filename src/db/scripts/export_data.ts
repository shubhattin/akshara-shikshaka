...
const main = async () => {
  ...
  */
  ...
  dbClient_ext.transaction(async (tx) => {
    ...
    try {
      await tx.delete(text_gestures);
      await tx.delete(text_lessons);
      await tx.delete(gesture_text_key_category_join);
      await tx.delete(gesture_categories);
      await tx.delete(lesson_categories);
      await tx.delete(lesson_gestures);
      await tx.delete(text_lesson_words);
      await tx.delete(audio_assets);
      await tx.delete(image_assets);
      await tx.delete(user_gesture_recordings);
      await tx.delete(user_gesture_recording_vectors);
    ...
    try {
      await tx.insert(audio_assets).values(data.audio_assets);
      console.log(
    ...
    try {
      await tx.insert(gesture_categories).values(data.gesture_categories);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx
        ...
        .values(
          /* SAFETY: validated at boundary - type assertion is safe based on prior schema check */ data.text_gestures as InferInsertModel<
            typeof text_gestures
          >[]
        );
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(gesture_text_key_category_join).values(data.gesture_text_key_category_join);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(lesson_categories).values(data.lesson_categories);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(text_lessons).values(data.text_lessons);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(lesson_gestures).values(data.lesson_gestures);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(text_lesson_words).values(data.text_lesson_words);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(user_gesture_recordings).values(data.user_gesture_recordings);
      console.log(
        chalk.green('✓ Successfully added values into table'),
    ...
    try {
      await tx.insert(user_gesture_recording_vectors).values(data.user_gesture_recording_vectors);
      console.log(
        chalk.green('✓ Successfully added values into table'),
